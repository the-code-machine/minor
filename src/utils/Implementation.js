...
export const SendImplementation = () => {
    ...
    const [uploadImplementation, setUploadImplementation] = useState('');
    ...
    const updateProject = async (e) => {
        ...
        ImplementationData[responsibilityCamelCase] = [{ work: coverLetter, documentUrl: attachedFile }];


        const updateData = {
            implementation: synopsisData,
        };

        if (team?.leaderId === user?.userId) {
            updateData.ImplementationDocument = uploadImplementation;
        }
        try {
                ...
                <form onSubmit={updateProject}>
                    ...
                    <div className="mb-5.5">
                        <label htmlFor="coverLetter" className="mb-3 block text-sm font-medium text-black dark:text-white">
                            Describe your responsibility as a {Responsiblity()} in Implementation Submission
                        </label>
                        <textarea
                            className="w-full rounded border border-stroke bg-[#C5F2DD] py-3 px-4 text-black focus:border-black focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white"
                            rows={6}
                        ...
                        <div className="mb-5.5 flex flex-col gap-5.5 sm:flex-row">
                            <div className="w-full ">
                                ...
                                <input
                                    type="text"
                                    className="w-full rounded border border-stroke bg-[#C5F2DD] py-3 px-4 text-black focus:border-black focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white"
                                    value={uploadImplementation}
                                    onChange={(e) => setUploadImplementation(e.target.value)}
                                    id="uploadSynopsis"
                                    placeholder="https://drive.google.com/...."
                                />
                            </div>
