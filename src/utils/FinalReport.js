...
export const SendFinalReport= () => {
    ...
    const [uploadFinalReport, setUploadFinalReport] = useState('');
    ...
    const updateProject = async (e) => {
        ...
        finalReportData[responsibilityCamelCase] = [{ work: coverLetter, documentUrl: attachedFile }];
        ...
        if (team?.leaderId === user?.userId) {
            updateData.FinalReportDocument = uploadFinalReport;
        }
        try {
                ...
                <form onSubmit={updateProject}>
                    ...
                    <div className="mb-5.5">
                        <label htmlFor="coverLetter" className="mb-3 block text-sm font-medium text-black dark:text-white">
                            Describe your responsibility as a {Responsiblity()} in Final Report Submission
                        </label>
                        <textarea
                            className="w-full rounded border border-stroke bg-[#C5F2DD] py-3 px-4 text-black focus:border-black focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white"
                            rows={6}
                        ...
                        <div className="mb-5.5 flex flex-col gap-5.5 sm:flex-row">
                            <div className="w-full ">
                                <label htmlFor="uploadSynopsis" className="mb-3 block text-sm font-medium text-black dark:text-white">
                                    Upload FinalReport Document
                                </label>
                                <input
                                    type="text"
                                    className="w-full rounded border border-stroke bg-[#C5F2DD] py-3 px-4 text-black focus:border-black focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white"
                                    value={uploadFinalReport}
                                    onChange={(e) => setUploadFinalReport(e.target.value)}
                                    id="uploadSynopsis"
                                    placeholder="https://drive.google.com/...."
                                />
                            </div>
