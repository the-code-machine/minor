...
export const Grading = () => {
  return (
    <>
      <Breadcrumb pageName={"Grade Panel"} />
      <TableOne />
    </>
  );
};
            ...
            <tr key={key} className="py-5">
              {tableHead.map((header, idx) => (
                <td key={idx} className="p-2.5 xl:p-5 text-center">
                  <Link href={`/dashboard/mentor/project/${tableData[0].teamid}`}  className="lg:text-xl hover:text-white hover:bg-black text-md font-semibold border-2 bg-transparent text-black border-black rounded p-2 w-full dark:text-white flex items-center justify-center">
